...
%%% IV 2023
clear, clc
cd /Users/lizheng/Desktop/同步文件夹/博士研究课题/OHBM会议数据分析/Version4/output/
% train /Users/lizheng/Desktop/同步文件夹/博士研究课题/OHBM会议数据分析/Version2/output
% Import data
...
for x = 1:length(permp)
    if permp(x) >= 0.05
        ...
        % Flip LV1, LV3, LV4, and LV6 and LV7 x == 3 | x == 4 | x == 6
        if x == 1 | x == 3 | x == 4 | x == 5 | x == 6
        ...
        writematrix(edgedata_sig, [[outname '/sig_edges_sympLV'], num2str(x), '_sigonly.csv'])
        writematrix(edgedata_raw, [[outname '/sig_edges_sympLV'], num2str(x), '_raw.csv'])

        % Prep edge weights output
        [edge_ordered, i] = sort(edgeweights(:,1), 'descend', 'ComparisonMethod','abs');
        
        labels = readtable("she400.txt", ...
    'FileType', 'text', ...
    'ReadVariableNames', false, ...
    'TextType', 'string', ...
    'Delimiter', 'none'); 
        labelname = table2cell(labels);

      

        edge = table(edgeweights_sig, labelname);

        %writematrix(edgedat, ['symptom_output/edge_weights_matrixformat_symp_LV' num2str(x), '.csv'])
        writetable(edge, [[outname '/edge_weights_ordered_sig_sympLV'], num2str(x), '.csv'])
        ...
        writetable(behav, [[outname '/symptom_weights_ordered_sig_sympLV'], num2str(x), '.csv'])
        ...
        writetable(behavcorrs, [[outname '/symptom_loadings_ordered_sig_sympLV'], num2str(x), '.csv'])
