...
```{r}
## Set paths
filepath <- "/Users/lizheng/Desktop/同步文件夹/博士研究课题/OHBM会议数据分析/Version3/behavior_data/"
outpath <- "/Users/lizheng/Desktop/同步文件夹/博士研究课题/OHBM会议数据分析/Version3/output/"
```

```{r}
## Load data
CBCL <- fread(paste0(filepath, "CBCL.csv"))
SDQ <- fread(paste0(filepath, "SDQ.csv"))
...
# 修改SDQ的第一列
SDQ$Identifiers <- str_remove(SDQ$Identifiers, ",assessment")

SDQ_col_names <- names(SDQ)[2:ncol(SDQ)]

SDQ_new_col_names <- str_sub(SDQ_col_names, start = 6)

names(SDQ)[2: ncol(SDQ)] <- SDQ_new_col_names 
...
MSN_names <- c(names(MSN_vovars)[2:361], "TIV" ,"Identifiers", "study_site", "age", "sex", "euler")
...
# 计算目标列中0的数量占总数量的比例
zero_ratios <- sapply(SDQ_col_names, function(col_name) {
  # 提取当前列数据
  col_data <- merged_data[[col_name]]
  # 统计0的数量（忽略NA）
  zero_count <- sum(col_data == 0, na.rm = TRUE)
...
SDQ_col_names <- names(SDQ)[2:ncol(SDQ)]
SDQ_new <- merged_data %>%
  select(all_of(c("Identifiers" , SDQ_col_names)))

SDQ_new$missing_ratio <- rowSums(is.na(SDQ_new[, 2:ncol(SDQ_new)])) / ncol(SDQ_new[, 2:ncol(SDQ_new)])
         

# 按缺失值比例从高到低排序（desc()表示降序）
sorted_result <- SDQ_new %>%
  # 保留被试标识符和缺失值占比列（按需选择，也可保留全部列）
  select(Identifiers, missing_ratio) %>%
  arrange(desc(missing_ratio))  # desc()：降序排列
...
# 过滤掉缺失值占比>10%的行，保留剩余行
SDQ_filtered <- SDQ_new %>%
  filter(missing_ratio <= 0.1)  # 只保留缺失值≤10%的被试

cat("过滤前样本量：", nrow(CBCL_new), "\n")
cat("过滤后样本量：", nrow(SDQ_filtered), "\n")

```
## 识别MSN缺失值的情况
```{r}

target_cols <- c("Identifiers", names(MSN_vovars)[2:361], "TIV" , "study_site", "age", "sex", "euler")
MSN_new <- merged_data %>%
...
```{r}
 # merge CBCL diagnosis MSN data
# 以selected_MSN为基准（左表），依次合并另外两个数据集
fin_data<- MSN_new_filtered %>%
  # 左连接selected_diagnosis：保留selected_MSN的所有样本，匹配diagnosis数据
  inner_join(CBCL_imputed, by = "Identifiers") %>%
  # 继续左连接CBCL：保留前面结果中的所有样本（即selected_MSN的样本），匹配CBCL数据
...
# 确认行数与基准数据集一致（应等于selected_MSN的行数）
cat("基准数据集(CBCL_imputed)行数：", nrow(MSN_new_filtered), "\n")
