...
```{r}
filepath <- "D:/Filez/李征/Sync/同步文件夹/博士研究课题/OHBM会议数据分析/behavior_data/output/"
outpath <- "D:/Filez/李征/Sync/同步文件夹/博士研究课题/OHBM会议数据分析/behavior_data/output/"
```

```{r}
## Load data
covars <- fread(paste0(filepath, "all_data.csv"))
select_col <- c('Identifiers', 'sex', 'age', "DX_01", "DX_01_Cat")
data_split <- covars %>%
...
data_split$strata_factor <- interaction(
  ...
  data_split$sex,
  data_split$DX_01_Cat,  # 即你之前处理的DX_01_Cat
  drop = TRUE  # 去除无数据的组合
)
